...
# ====================== VERSION CONTROL ======================
VERSION = "v3.8"  # Restricted Health now limited by team (per-user)
...
if authentication_status is True:
    ...
    # User roles + NEW restricted teams permission
    ...
    # NEW: Restricted Teams permission
    ...
    can_see_all_teams = not allowed_teams or "All" in allowed_teams or is_admin
    ...
    # ====================== RESTRICTED HEALTH PAGE (NEW LOGIC) ======================
    if page == "🔒 Restricted Health":
        if can_restricted:
            st.header("🔒 Restricted Health Data")

            # Build team dropdown based on user's allowed teams
            ...
            if selected_team == "All Teams":
                ...
                roster = players_df[players_df.get("Division", "") == selected_team].copy()   # using Division from your form
            ...
            if not roster.empty:
                ...
                for idx, player in roster.iterrows():
                    ...
                    with st.expander(f"{player['First Name']} {player['Last Name']} {'⚠️ ' + alert_text if alert_text else ''}"):
                        ...
                        # ... (rest of medical fields as before)
            else:
                st.info("No players found for the selected team.")
        else:
            st.warning("🔒 Restricted access denied.")

    # All other pages remain exactly as in v3.3 (Players, Registrar, Equipment, Events, Admin, Profile)
    # (Full code for those pages is unchanged from your stable version)
